...
function oi_page_handler($page) {
	...
	switch ($page[0]) {
		...
		case 'join':
			oi_pagehandler_join($page);
			break;
			
			
		case 'admin':
			oi_pagehandler_admin($page);
			break;
	}
}

/**
 * Helper functions to de-clutter the main page handler.
 * 
 * @param $page
 * @return str
 */
function oi_pagehandler_join($page) {
	// only for new users.
	if (isloggedin()) {
		forward();
	}
...
/**
 * Helper function for main pagehandler.
 * 
 * @param $page
 * @return str
 */
function oi_pagehandler_admin($page) {
	admin_gatekeeper();
